...
results_folder_name = "Results_uu"
...
for i in range(n_combinations):
    ...
    t, t_max = 0, 250_000  # number of iterations - time steps
    past_mean_pheno = 1
    mu = 0  # general mean of the different processes

    initial_spite = 0  # initial spite genotype of individuals
    ...
    spite_resistance = True  # determines if the value of the spite genotype is added to the individual's kd
    ...
    dir_name = f"d{diff_sigma}_c{comp_sigma}_s{spite_sigma}_cost{cost}_kd{kd}_res{spite_resistance}_kc{physical_kc}"
    ...
    if not os.path.exists(f"{results_folder_name}/{dir_name}"):
        ...
        if not overwrite_dir_results:
            print("Experiment folder already exists. Please change the experiment name to prevent overwriting or set "
                  "overwrite_dir_results to True.")
            sys.exit()
    ...
    def first_individuals(N, initial_spite):
        """Generate the first N individuals at random positions with the initial spite genotype"""
        ...
        individuals['y'] = y
        individuals['w'] = 1
        individuals['p'] = initial_spite
        return individuals


    def report(individuals, t, stats):
        """ At every time-step, save the time, the number of individuals, the mean
        and max phenotype, the standard deviation of the phenotype, the covariance between
        ...
        max_pheno = np.max(individuals['p'])
        ...
        stats['maxp'][t] = max_pheno
        stats['std'][t] = np.std(individuals['p'])
        stats['s'][t] = cov_w
        ...
        if t % (t_max / 1000) == 0:
            np.savez('%s/stats' % path, t=stats['t'], n=stats['n'], mp=stats['mp'], maxp=stats['maxp'],
                     std=stats['std'], s=stats['s'], e=stats['e'], d=stats['d'], maxd=stats['maxd'], r=stats['r'])
    ...
    def pheno_sum(individuals):
        """Sum the phenotype at each position. Get the coordinates of each individual and add the value of its
        phenotype to the corresponding position."""
        field_pheno = np.zeros((XY, XY))
        x = individuals['x']
        y = individuals['y']
        # convert negative spite genotypes to zero
        p = np.clip(individuals['p'], a_min=0, a_max=None)
        # Vectorized operation with "fancy indexing", add corresponding spite to each position for each (x,y) coordinate
        np.add.at(field_pheno, (x, y), p)
    ...
    def mutation(individuals, nnew):
        ...
        for i in individuals[(len(individuals) - nnew):]:
            if 0 < prob[n] <= pmut:
                i['p'] += mut_step
            # same probability (equal density) to increase and to decrease
            elif pmut < prob[n] <= 2 * pmut:
                i['p'] -= mut_step
            n += 1
        # The phenotype is allowed to become negative, but is clipped at zero for growth and death rate calculations
    ...
    def growth(individuals):
        ...
        # convert negative spite genotypes to zero
        p = np.clip(individuals['p'], a_min=0, a_max=None)
        metabolic_cost = np.clip(1 - cost * p, a_min=0, a_max=None)  # If spite cost is too high, convert to zero
        growth_rates = g0 * (1 - position_comp_densities / kc) * metabolic_cost
    ...
    def death(individuals, t):
        """Remove individuals depending on their death rate. The death_rate is calculated with the density of the
        phenotype of individuals at its position and other parameters. Obtain the indices of the unlucky individuals
        and delete them from the array. A report of all individuals is made before any is removed"""

        # Apply the kernel fourier transform, then
        # multiply them and do the inverse transform to the result to get the density
        pheno = pheno_sum(individuals)
        ...
        # Without subtracting p * position0_spite, the individual's spite affects itself too (Selfspite)
        position_pheno_density = pheno_density[individuals['x'], individuals['y']]
        # To exclude the individual contribution to its own spite density, use the following line instead:
        # position_pheno_density = position_pheno_density - p * position0_spite

        individuals['d'] = position_pheno_density

        if kd == 'i':  # infinite resistance to spite
            death_rate = d0
        elif spite_resistance:
            death_rate = d0 * (1 + position_pheno_density / (kd + p))  # (kd + individuals['p'])
        else:
        ...
        # Get a list of individual indices to kill
        prob = np.random.uniform(size=len(individuals))
        idx = death_rate > prob
    ...
    dt_s = np.dtype([('t', np.int32), ('n', np.int32), ('mp', np.float32), ('maxp', np.float32),
                     ('std', np.float32), ('s', np.float32), ('e', np.float32), ('d', np.float32),
                     ('maxd', np.float32), ('r', np.float32)])  # Variables type definition
    stats = np.zeros((t_max + 1, 1), dtype=dt_s)

    # Generate the first individuals
    individuals = first_individuals(N, initial_spite)
    ...
    while t <= t_max and len(individuals) != 0:
        ...
        if t % 1000 == 0:
            print(f"time: {t}, mean spite: {np.mean(individuals['p'])}, "
                  f"max spite: {max(individuals['p'])}, n_individuals: {len(individuals)}")
        t += 1
